...
FROM Supplier AS S
WHERE S.id IN 
(SELECT P.Supplierid FROM Product AS P;)
...
FROM [dbo].[Order] 
JOIN [dbo].[OrderItem] 
ON [dbo].[Order].Id= [dbo].[OrderItem].OrderId;
...
SELECT FirstName,LastName, City, Country, Phone, [Order].Id,OrderNumber
FROM [dbo].[Customer]
JOIN [dbo].[Order]
...
SELECT ProductName , Quantity, [OrderItem].UnitPrice AS Price
FROM [dbo].[Product]
JOIN [dbo].[OrderItem]
...
SELECT ProductName, ProductAvaiable =
CASE
...
SELECT CompanyName, ContactName, Country,Langua=
CASE
...
WHEN Country = 'Norway' THEN 'Norwegian and Sami'
WHEN Country = 'Singapore' THEN 'English,Malay,Mandarin and Tamil'
WHEN Country = 'Spain' THEN 'Spanish or Castillian Spanish'
...
FROM [dbo].[Product]
WHERE [dbo].[Product].Package LIKE '%jars';
...
SELECT CompanyName, COUNT(ProductName) AS No_Of_Product
FROM [dbo].[Supplier]
JOIN [dbo].[Product]
ON [dbo].[Supplier].Id = [dbo].[Product].SupplierId
GROUP BY CompanyName
ORDER BY CompanyName DESC;

---13/List the number of customers in each country.

SELECT Country, COUNT(LastName) AS NO_Customer
FROM Customer
GROUP BY Country
ORDER BY Country;
...
SELECT FirstName, LastName, Sum(TotalAmount) AS TotalOrderAmount
FROM [dbo].[Customer]
JOIN[dbo].[Order]
ON [dbo].[Customer].Id = [dbo].[Order].CustomerId
...
SELECT Country,COUNT(*) AS No_Of_Supplies
FROM Supplier
GROUP BY Country
HAVING COUNT(*) > 2;
...
SELECT FirstName, LastName, avg(TotalAmount) AS AvgOrder
FROM [dbo].[Customer]
JOIN [dbo].[Order]
ON [dbo].[Customer].Id = [dbo].[Order].CustomerId
GROUP BY FirstName, LastName
...
SELECT OrderDate, COUNT(*) AS No_Of_Order, SUM(TotalAmount) AS Total_Amount_Sold
FROM [dbo].[Order]
